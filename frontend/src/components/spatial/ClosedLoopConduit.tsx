...
export default function ClosedLoopConduit() {
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="bg-[#FFFFFF] p-6 rounded-2xl border border-[#2B2D42]/10 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <h3 className="text-xl font-bold text-[#0B2545]">The Closed-Loop Transformation Conduit</h3>
          </div>
          <div className="flex items-center space-x-2 bg-[#1F7A8C]/10 px-3 py-1.5 rounded-full border border-[#1F7A8C]/20 text-xs font-bold text-[#1F7A8C]">
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isSelected
                    ? "bg-[#1F7A8C] text-[#FFFFFF] border-[#1F7A8C] shadow-xs font-bold"
                    : stage.status === 'completed'
                    ? "bg-[#2E7D32]/10 border-[#2E7D32]/30 text-[#2E7D32] hover:border-[#2E7D32]"
                    : stage.status === 'active'
                    ? "bg-[#1F7A8C]/10 border-[#1F7A8C] text-[#1F7A8C] hover:bg-[#1F7A8C]/15"
                    : "bg-[#FFFFFF] border-[#2B2D42]/10 text-[#2B2D42]/60 hover:bg-[#F4F6F9] hover:text-[#0B2545]"
                )}
              >
                <div className="flex justify-center mb-1.5">
                  {stage.status === 'completed' ? (
                    <CheckCircle2 className={cn("w-4 h-4", isSelected ? "text-[#FFFFFF]" : "text-[#2E7D32]")} />
                  ) : stage.status === 'active' ? (
                    <Circle className={cn("w-4 h-4", isSelected ? "text-[#FFFFFF]" : "text-[#1F7A8C] animate-pulse")} />
                  ) : (
                    <Circle className="w-4 h-4 text-[#2B2D42]/30" />
                  )}
                </div>
                <div className="text-[11px] font-bold tracking-wider">{stage.label}</div>
                <div className={cn("text-[9px] truncate mt-0.5", isSelected ? "text-[#FFFFFF]/90" : "text-[#2B2D42]/60")}>
                  {stage.sublabel}
                </div>
              </div>
            );
      ...
      {activeStage && (
        <div className="p-6 bg-[#FFFFFF] rounded-2xl border border-[#2B2D42]/10 shadow-xs space-y-4 animate-in fade-in duration-200">
          {(() => {
            const current = stages.find(s => s.id === activeStage)!;
            return (
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#2B2D42]/10 pb-3">
                  <div>
                    <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-[#1F7A8C]">
                      PHASE 0{current.id} // {current.label}
                    </span>
                    <h4 className="text-lg font-bold text-[#0B2545]">{current.sublabel}</h4>
                  </div>
                  {current.metric && (
                    <span className="text-xs font-mono font-bold px-3 py-1 rounded-lg bg-[#F4F6F9] border border-[#2B2D42]/10 text-[#0B2545] shadow-xs">
                      {current.metric}
                    </span>
                  )}
                </div>

                <p className="text-xs text-[#2B2D42] leading-relaxed max-w-3xl">
                  {current.detail}
                </p>

                {current.igotModule && (
                  <div className="p-4 rounded-xl bg-[#FFFFFF] border border-[#2B2D42]/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 rounded-lg bg-[#1F7A8C]/10 flex items-center justify-center text-[#1F7A8C] shrink-0">
                        <BookOpen className="w-5 h-5" />
                      </div>
                      <div>
                        <div className="text-[10px] font-bold uppercase text-[#1F7A8C] font-mono">Integrated Course Module</div>
                        <h5 className="font-bold text-xs text-[#0B2545]">{current.igotModule}</h5>
                      </div>
                    </div>
                    {current.actionRoute && (
                      <Link to={current.actionRoute}>
                        <Button size="sm" className="font-bold bg-[#1F7A8C] hover:bg-[#1F7A8C]/90 text-[#FFFFFF] shadow-xs">

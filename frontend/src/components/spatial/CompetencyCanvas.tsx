...
export default function CompetencyCanvas({ nodes, selectedNode, onSelectNode, className }: Props) {
  ...
  return (
    <div className={cn("relative w-full h-[620px] bg-[#FFFDF9] rounded-2xl border border-[#292B2B]/10 overflow-hidden select-none shadow-sm flex flex-col justify-between", className)}>
      {/* 1. Clean Structured Zone Column Headers (No Overlap) */}
      <div className="grid grid-cols-3 border-b border-[#292B2B]/10 bg-[#EFEBE4]/60 px-4 py-2.5 z-10">
        <div className="text-left text-[11px] font-mono font-bold text-[#2D3030] uppercase tracking-wider">
          Zone 1 // Theory & Inference
        </div>
        <div className="text-center text-[11px] font-mono font-bold text-[#2D3030] uppercase tracking-wider border-x border-[#292B2B]/10 px-2">
          Zone 2 // Field & Sampling
        </div>
        <div className="text-right text-[11px] font-mono font-bold text-[#2D3030] uppercase tracking-wider">
      ...
      <div className="relative flex-1 w-full h-full">
        ...
        <div className="absolute inset-0 grid grid-cols-3 pointer-events-none">
          <div className="border-r border-dashed border-[#292B2B]/5" />
          <div className="border-r border-dashed border-[#292B2B]/5" />
          <div />
        </div>
            ...
            return (
              <g key={idx}>
                ...
                <line
                  ...
                  x2={`${target.x}%`}
                  y2={`${target.y}%`}
                  stroke={isHighlighted ? (hasGapFlow ? "#B38A3D" : "#A85D4C") : "#292B2B"}
        ...
        {nodes.map((node) => {
          ...
          return (
            ...
            >
              ...
              {node.id === 3 && (
                <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-[#2D3030] text-[#FFFDF9] text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full shadow-sm flex items-center space-x-1 whitespace-nowrap border border-[#2D3030] z-30">
                  <User className="w-3 h-3 text-[#B38A3D]" />
              ...
              {/* Node Card Surface */}
              <div className={cn(
                "w-44 sm:w-48 px-3.5 py-2.5 rounded-xl border flex items-center space-x-3 bg-[#FFFDF9] transition-all shadow-xs text-left",
                isSelected 
                  ? "border-[#A85D4C] ring-2 ring-[#A85D4C]/20 shadow-md" 
                  : isHovered 
                  ? "border-[#A85D4C]/60 shadow-xs" 
                  : isCritical 
                  ? "border-[#B38A3D]/50" 
                  : "border-[#292B2B]/15"
              )}>
                ...
                <div className="relative flex items-center justify-center shrink-0">
                  <div className={cn(
                    ...
                    isTargetMet 
                      ...
                      : isCritical
                      ? "bg-[#B38A3D]/15 text-[#2D3030] border-[#B38A3D]/40"
                ...
                <div className="flex-1 min-w-0">
                  <h4 className={cn(
                    ...
                    isSelected ? "text-[#A85D4C]" : "text-[#2D3030]"
                  )}>
                    {node.name}
                  </h4>
                  <div className="flex items-center space-x-1.5 mt-0.5 text-[10px] font-mono">
                    <span className="text-[#292B2B]/60">Req: {node.required}%</span>
                    {node.gap > 0 ? (
                      <span className="font-bold px-1.5 py-0.2 rounded bg-[#B38A3D]/15 text-[#2D3030] border border-[#B38A3D]/30">
                        -{node.gap}%
                      </span>
                    ) : (
      ...
      <div className="border-t border-[#292B2B]/10 bg-[#EFEBE4] px-5 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs font-mono text-[#292B2B] z-10">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-1.5">
            <span className="w-2.5 h-2.5 rounded-full bg-[#A85D4C]" />
            <span className="text-[11px] font-semibold">On Track</span>
          </div>
          <div className="flex items-center space-x-1.5">
            <span className="w-2.5 h-2.5 rounded-full bg-[#B38A3D]" />
            <span className="text-[11px] font-semibold">Priority Gap</span>
          </div>
        </div>

        <div className="text-[11px] text-[#A85D4C] font-bold">

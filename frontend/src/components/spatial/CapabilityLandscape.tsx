...
export default function CapabilityLandscape({ nodes, selectedNode, onSelectNode, className }: Props) {
  ...
  const getNodeColor = (node: CapabilityNode) => {
    if (node.score === null || node.status === 'not_assessed') {
      return { ring: '#2B2D42', bg: 'bg-[#2B2D42]/5', text: 'text-[#2B2D42]/60', border: 'border-[#2B2D42]/20' };
    }
    if (node.score >= 80) return { ring: '#2E7D32', bg: 'bg-[#2E7D32]/10', text: 'text-[#2E7D32]', border: 'border-[#2E7D32]/30' };
    if (node.score >= 65) return { ring: '#1F7A8C', bg: 'bg-[#1F7A8C]/10', text: 'text-[#1F7A8C]', border: 'border-[#1F7A8C]/30' };
    return { ring: '#D4AF37', bg: 'bg-[#D4AF37]/15', text: 'text-[#0B2545]', border: 'border-[#D4AF37]/40' };
  };

  return (
    <div className={cn("relative w-full h-[540px] sm:h-[580px] bg-[#FFFFFF] rounded-2xl border border-[#2B2D42]/10 overflow-hidden select-none shadow-xs flex flex-col justify-between", className)}>
      {/* Topology Header */}
      <div className="flex items-center justify-between border-b border-[#2B2D42]/10 bg-[#F4F6F9]/60 px-5 py-3 z-10">
        <div>
        ...
        <div className="flex items-center space-x-3 text-[11px] font-mono text-[#2B2D42]/70 hidden sm:flex">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#2E7D32]" /> Verified (≥80%)</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#1F7A8C]" /> On Track</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#D4AF37]" /> Needs Attention</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#2B2D42]/30" /> Not Assessed</span>
        </div>
      </div>
      ...
      <div className="relative flex-1 w-full h-full">
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none">
          ...
          <circle cx="50%" cy="50%" r="28%" fill="none" stroke="#2B2D42" strokeOpacity="0.05" strokeDasharray="3 3" />
          <circle cx="50%" cy="50%" r="42%" fill="none" stroke="#2B2D42" strokeOpacity="0.03" />
            ...
            return (
              <g key={`edge-${idx}`}>
                <line
                  ...
                  x2={`${target.x}%`}
                  y2={`${target.y}%`}
                  stroke={isRelated ? "#1F7A8C" : "#2B2D42"}
        ...
        >
          <div className="w-16 h-16 rounded-full bg-[#0B2545] border-2 border-[#1F7A8C] text-[#FFFFFF] flex flex-col items-center justify-center shadow-md ring-4 ring-[#1F7A8C]/20">
          ...
          return (
            ...
            >
              ...
              <div className={cn(
                ...
                isSelected 
                  ? "border-[#1F7A8C] ring-4 ring-[#1F7A8C]/20 shadow-md" 
                  : isHovered 
              ...
              <div className={cn(
                ...
                isSelected 
                  ? "border-[#1F7A8C] bg-[#1F7A8C]/5 shadow-xs" 
                  : "border-[#2B2D42]/10"
              )}>
                <span className="text-[11px] font-bold text-[#0B2545] whitespace-nowrap block">
                  {node.name}
                </span>
                <span className="text-[9px] font-mono text-[#2B2D42]/60 uppercase tracking-tighter block -mt-0.5">
      ...
      {/* Footer Info */}
      <div className="border-t border-[#2B2D42]/10 bg-[#F4F6F9]/40 px-5 py-2.5 text-[11px] font-mono text-[#2B2D42]/70 flex items-center justify-between z-10">
        <span>Framework: MoSPI Statistical Official Standards</span>
        <span className="text-[#1F7A8C] font-semibold flex items-center gap-1">

...
export default function GapEngineFlow() {
  ...
  const steps = [
    {
      ...
      accent: 'border-[#B38A3D]',
      pill: 'bg-[#B38A3D]/15 text-[#2D3030] border-[#B38A3D]/30'
    },
    {
      id: 2,
      tag: 'STAGE 02',
      ...
      accent: 'border-[#A85D4C]',
      pill: 'bg-[#A85D4C]/10 text-[#A85D4C] border-[#A85D4C]/20'
    },
    {
      id: 3,
      tag: 'STAGE 03',
  ...
  return (
    <section id="gap-engine" className="py-20 px-4 sm:px-6 lg:px-8 border-b border-[#292B2B]/10 bg-[#FFFDF9] relative overflow-hidden">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-1/2 left-8 right-8 h-1 bg-[#292B2B]/10 -translate-y-6 z-0" />
            ...
            {steps.map((s, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "p-5 rounded-2xl border transition-all duration-300 cursor-pointer text-left flex flex-col justify-between space-y-4 bg-[#FFFDF9]",
                    isSelected 
                      ? "ring-2 ring-[#A85D4C] border-[#A85D4C] shadow-md -translate-y-1" 
                      : "border-[#292B2B]/10 hover:border-[#A85D4C]/50 shadow-xs"
                  )}
                >
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {isLast ? (
                        ...
                        <span className="text-xs font-mono font-bold text-[#292B2B]/40">#{s.id}</span>
                      )}
                    </div>
                    ...
                    <div>
                      <span className={cn(
                        ...
                        s.id === 1 ? "text-[#2D3030]" : s.id === 5 ? "text-[#2E8B57]" : "text-[#A85D4C]"
                      )}>
                        {s.metric}
                      </span>
                      <h4 className="text-xs font-bold text-[#2D3030] mt-0.5">{s.title}</h4>
                      <p className="text-[11px] font-mono text-[#292B2B]/60 mt-0.5">{s.subtitle}</p>
                    </div>
                  </div>

                  {/* Body description */}
                  <p className="text-xs text-[#292B2B]/80 leading-relaxed border-t border-[#292B2B]/10 pt-3">
                    {s.description}
                  </p>
                </div>
              );
        ...
        <div className="p-6 rounded-2xl bg-[#EFEBE4] border border-[#292B2B]/10 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center space-x-3 text-left">
            <div className="w-10 h-10 rounded-xl bg-[#2D3030] text-[#FFFDF9] flex items-center justify-center shrink-0">
              <Brain className="w-5 h-5 text-[#A85D4C]" />
            </div>
            <div>
              <h4 className="text-sm font-bold text-[#2D3030]">Adaptive Learning Loop in Action</h4>
              <p className="text-xs text-[#292B2B]/70">
                Step <strong>#{activeStep}: {steps[activeStep - 1].title}</strong> is active in this interactive preview.
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2 font-mono text-xs text-[#2D3030]">
            <span className="px-3 py-1 rounded-lg bg-[#FFFDF9] border border-[#292B2B]/10 font-bold">

  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto pb-16 text-left">
        ...
        <Card className="bg-[#FFFFFF] border-2 border-[#1F7A8C] shadow-md animate-in fade-in duration-200">
          ...
          <CardContent className="p-6">
            <form onSubmit={handleUploadLearnerNotes} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    className="text-xs border-[#2B2D42]/20"
              ...
              <div className="border-2 border-dashed rounded-xl p-6 text-center border-[#2B2D42]/20 hover:border-[#1F7A8C] transition-colors relative bg-[#F4F6F9]/50">
                <input
                  type="file"
                  accept=".pdf,.docx,.txt"
                ...
                <div className="flex flex-col items-center space-y-1.5 pointer-events-none">
                  ...
                  <p className="text-[10px] text-[#2B2D42]/60">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {myNotes.map((note) => {
              ...
              return (
                <Card key={note.id} className="bg-[#FFFFFF] border border-[#2B2D42]/15 shadow-xs flex flex-col justify-between hover:border-[#1F7A8C]/40 transition-all">
                  <CardContent className="p-5 space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <h4 className="font-bold text-xs sm:text-sm text-[#0B2545] leading-snug">
                            {note.title}
                          </h4>
                          <p className="text-[10px] text-[#2B2D42]/50 font-mono">
                    ...
                    <div className="p-3 rounded-xl bg-[#F4F6F9] border border-[#2B2D42]/10 space-y-1">
                      <div className="flex items-center space-x-1.5 text-[11px] font-mono font-bold text-[#1F7A8C]">
                    ...
                    {note.topics && note.topics.length > 0 && (
                      <div className="flex flex-wrap gap-1 pt-1">
                        {note.topics.map((t: string, tIdx: number) => (
                          <span key={tIdx} className="px-2 py-0.5 rounded bg-[#1F7A8C]/10 text-[#1F7A8C] text-[10px] font-bold border border-[#1F7A8C]/20 font-mono">
                            {t}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="p-4 pt-0 border-t border-[#2B2D42]/10 mt-2 flex flex-col sm:flex-row items-center gap-2">
                    <Button
                      size="sm"
                      onClick={() => handleGenerateNotesQuiz(note.id, 3)}
                      disabled={isGenerating}
                      className="w-full sm:flex-1 bg-[#1F7A8C] hover:bg-[#1F7A8C]/90 text-[#FFFFFF] font-bold text-xs shadow-2xs cursor-pointer"
                    >
                    ...
                    <Button
                      ...
                      disabled={isGenerating}
                      className="w-full sm:flex-1 border-[#2B2D42]/20 hover:bg-[#F4F6F9] text-[#0B2545] font-bold text-xs cursor-pointer"
                    >
      ...
      <Card className="bg-[#FFFFFF] shadow-xs border border-[#2B2D42]/10">
        <CardHeader className="bg-[#F4F6F9] border-b border-[#2B2D42]/10 p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              <div className="p-2 rounded-xl bg-[#1F7A8C] text-[#FFFFFF] shadow-2xs">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-[#2B2D42]/10">
            ...
            <Button
              ...
              className="w-full sm:w-auto bg-[#1F7A8C] hover:bg-[#1F7A8C]/90 text-[#FFFFFF] font-bold text-xs shadow-xs cursor-pointer px-6"
            >
              {startingPractice ? (
                <div className="flex items-center gap-2">
                  <div className="w-3.5 h-3.5 border-2 border-[#FFFFFF] border-t-transparent rounded-full animate-spin" />
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base sm:text-lg font-bold text-[#0B2545] flex items-center gap-2">
            <BookOpen className="w-4 h-4 text-[#1F7A8C]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {materials.map((m, idx) => {
            ...
            return (
              <Card key={m.id || idx} className="bg-[#FFFFFF] border border-[#2B2D42]/10 shadow-xs hover:border-[#1F7A8C]/50 transition-all flex flex-col justify-between">
                <CardContent className="p-5 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="p-2 rounded-lg bg-[#1F7A8C]/10 text-[#1F7A8C]">
                      <FileText className="w-4 h-4" />
                    </div>
                    <span className="text-[10px] font-mono text-[#2B2D42]/50 font-bold">
                      {m.upload_date || 'MoSPI 2026'}
                    </span>
                  </div>

                  <div>
                    <h4 className="font-bold text-xs sm:text-sm text-[#0B2545] leading-snug line-clamp-2">
                      {m.title}
                    </h4>
                    <p className="text-[11px] text-[#2B2D42]/70 mt-1 line-clamp-2">
                  ...
                  <div className="flex flex-wrap gap-1 pt-1">
                    {topics.slice(0, 2).map((t: string, tIdx: number) => (
                      <span key={tIdx} className="px-2 py-0.5 rounded bg-[#F4F6F9] text-[#2B2D42] text-[10px] font-medium border border-[#2B2D42]/10">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <div className="p-4 pt-0 border-t border-[#2B2D42]/5 mt-2 flex items-center gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleStartPractice(compId)}
                    className="flex-1 text-xs font-bold border-[#2B2D42]/20 hover:bg-[#1F7A8C] hover:text-[#FFFFFF] text-[#1F7A8C] transition-all cursor-pointer"
                  >

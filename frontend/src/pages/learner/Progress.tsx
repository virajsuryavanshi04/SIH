...
export default function Progress() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-16 text-left">
      ...
      <div className="bg-[#2D3030] rounded-2xl p-6 sm:p-8 text-[#FFFDF9] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 shadow-md border border-[#2D3030] relative overflow-hidden">
        ...
        <div className="text-left sm:text-right bg-[#FFFDF9]/10 p-4 sm:p-5 rounded-2xl border border-[#FFFDF9]/20 z-10 shrink-0">
          <span className="text-[11px] font-mono font-bold text-[#FFFDF9]/70 uppercase tracking-wider block">
            Net Capacity Gained
          </span>
          <div className="text-3xl sm:text-4xl font-extrabold text-[#FFFDF9] mt-0.5 font-mono">
            {totalGrowth > 0 ? `+${totalGrowth} pts` : `${totalGrowth} pts`}
          </div>
          <span className="text-[11px] font-mono text-[#B38A3D] block mt-0.5">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg sm:text-xl font-bold text-[#292B2B] flex items-center gap-2">
            <Layers className="w-4.5 h-4.5 text-[#A85D4C]" />
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {comp.subtopics.map((st: any) => (
                          <div key={st.topic_id} className="p-2.5 rounded-xl bg-[#EFEBE4] border border-[#E2DDD5] text-xs flex items-center justify-between">
                            <span className="truncate max-w-[140px] text-[#292B2B] font-medium" title={st.topic_name}>
                              {st.topic_name}
                            </span>
                            <span className={cn(

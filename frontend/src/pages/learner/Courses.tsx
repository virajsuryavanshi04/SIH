    ...
    return (
      <div className="min-h-[400px] flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs font-semibold text-[#123B5D]">Curating personalized iGOT & official learning resources...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-16 text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-[#176B87]/10 text-[#176B87] border border-[#176B87]/20 shadow-xs">
      ...
      {/* Search & Filter Bar */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 p-4 sm:p-5 bg-[#FFFFFF] rounded-2xl shadow-xs border border-[#D8E5EC]">
        <div className="relative">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#5D7180] w-4 h-4" />
          <Input 
            placeholder="Search resources, topics..." 
            className="pl-10 border-[#D8E5EC] focus:border-[#176B87] focus:ring-[#176B87]/20 bg-[#FFFFFF] text-sm font-medium text-[#123047] h-10 rounded-xl" 
            value={search} 
        ...
        <Select value={filter} onValueChange={setFilter}>
          <SelectTrigger className="border-[#D8E5EC] bg-[#FFFFFF] text-sm font-medium text-[#123047] h-10 rounded-xl">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg sm:text-xl font-semibold text-[#123047] flex items-center gap-2">
            <Layers className="w-5 h-5 text-[#176B87]" />
        ...
        {filteredCourses.length === 0 ? (
          <div className="p-12 text-center bg-[#FFFFFF] rounded-2xl border border-[#D8E5EC] space-y-3">
            <BookOpen className="w-8 h-8 text-[#5D7180]/30 mx-auto" />
            <p className="text-sm font-bold text-[#123047]">No matching learning resources found</p>
            <p className="text-xs text-[#5D7180]">Try adjusting your keyword filter or competency selection.</p>
          </div>
        ) : (
